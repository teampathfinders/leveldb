...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname)
    : env_(raw_options.env),
      ...
      suspending_compaction_(nullptr),
...
void DBImpl::MaybeScheduleCompaction() {
  ...
  } else if (imm_ == nullptr && suspending_compaction_.Acquire_Load()) {
...
void DBImpl::SuspendCompaction() {
	...
	mutex_.Lock();
	suspending_compaction_.Release_Store(this);  // Any non-nullptr value is ok
	mutex_.Unlock();
